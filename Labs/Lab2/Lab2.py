...
# https://www.tutorialspoint.com/scipy/scipy_integrate.html
...
mK=np.arange(-26,-16.9,.1)
print(mK[2]) #verifying our array
...
# ADD HERE
ax.semilogy(mK,schechter_M(mK,alpha=-1.35), color ='black', linestyle=':', linewidth=5, label= r'high $\alpha$')
ax.semilogy(mK,schechter_M(mK,alpha=-0.6), color ='green', linestyle='--', linewidth=5, label= r'low $\alpha$')
# Add labels
...
def schechter_L(lum,n_star=8e-3,l_star=1.4e10,alpha=-0.7):
    '''Funcgtion that computes the shcechter luminosity function for a given luminosity
    ...
    Inputs: 
        ...
        l_star : float
            Characteristic luminosity for the knee of the Schechter function) in units of Lsun 
        alpha: float
        Faint and slope 
    Outputs: 
        schechter_1:float 
            number density for the given luminosity (h^3 * Mpc^=3,Lsun)
...
#What fraction of the integrated luminosity density lies above L*
#Alpha==1
l_upper= quad(lambda L: L*schechter_L(L,alpha=1.0),1.4e10,1e14)
print(l_upper)
l_total= quad(lambda L: L*schechter_L(L,alpha=1.0),1.4e10,1e14)
print("Flux ratio(>L*)/Ltotal", np.round(l_upper[0]/l_total[0],3))
...
def salpeter(m,m_min=0.1,m_max=120,alpha=2.35):
    '''Function that defines the IMF. The function is normalized such that 
    it returns the fraction of stars expected at the given m (assuming stars range in
    mass from m_min to m_mas)
    Inputs: 
        m: array of floats
    ...
    Output: 
        norm_salpeter: float 
            normalized fraction of stars at a given mass m
    '''
    #Detemine the magnitude of the integral 
...
def salpeter_Mass(m,m_min=0.1,m_max=120,alpha=2.35):
    '''Function that defines the IMF. The function is normalized such that 
    it returns the fraction of mass of stars expected at the given m (assuming stars range in
    mass from m_min to m_mas)
    Inputs: 
        m: array of floats
            Array of stellar masses (Msun)
